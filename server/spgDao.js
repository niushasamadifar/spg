...
exports.getProducts = () => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT p.id, p.name, p.quantity, p.unit, p.farmer, f.name as farmerName, p.price FROM products p LEFT JOIN farmer f WHERE f.id = p.farmer ";
    db.all(sql, (err, rows) => {
      if (err) {
        reject(err);
        return;
...
exports.getClient = (id) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT id, name, surname, wallet, basket, email FROM users WHERE id = ?";
...
exports.addOrder = async (order) => {
  try {
    return new Promise((resolve, reject) => {
      const sql =
        "INSERT INTO orders (id, userID, products, address, date, time, amount, confPreparation) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
      db.run(
        sql,
        [
          order.id,
